import React, { createContext, useContext, useState, ReactNode, useEffect, useRef, useCallback, Dispatch, SetStateAction } from "react";
...
export const StoreProviderRequest: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  return (
    <StoreRequest.Provider value={{
      ...
      discardRequest: discardCursorRequest, defineRequest: defineFreeRequest,
...
interface RequestControllerType {
...
const useRequestController = ({ setData, pushEvent }: RequestControllerType) => {
  const { search, remove, excise } = useStoreCache();
  const { userData, fetchUser } = useStoreSession();
  const { fetchContext } = useStoreContext();
  const { push } = useAlert();

  const backupRef = useRef<ItemRequest>(newItemRequest(userData.username));
  const requestRef = useRef<ItemRequest>(newItemRequest(userData.username));
  const responseRef = useRef<ItemResponse>(newItemResponse(userData.username));
